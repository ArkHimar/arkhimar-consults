...
export function renderEmailHtml({subject='',preheader='',blocks=[],brand={}}={}){
  ...
  const content=blocks.map(block=>{
    ...
    if(block.type==='button')return`<p style="margin:28px 0"><a href="${escapeHtml(safeLink(block.url))}" target="_blank" rel="noopener noreferrer" style="display:inline-block;background:${b.primaryColor};color:#ffffff;text-decoration:none;padding:14px 22px;border-radius:3px;font-weight:700">${escapeHtml(block.label||'Continue').slice(0,120)}</a></p>`;
