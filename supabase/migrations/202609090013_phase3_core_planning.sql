...
begin
  ...
  for item in select value from jsonb_array_elements(wbs_items) loop
    ...
    values(coalesce(nullif(item->>'id','')::uuid,gen_random_uuid()),target_workspace,target_project,artifact_version.id,item->>'key',nullif(item->>'parentKey',''),item->>'outline',coalesce((item->>'sortOrder')::integer,0),coalesce(item->>'type','Work package'),item->>'name',coalesce(item->>'owner',''),greatest(0,coalesce((item->>'effort')::numeric,0)),greatest(0,coalesce((item->>'cost')::numeric,0)),coalesce(item->>'requirements',''),coalesce(item->>'deliverables',''),coalesce(item->'dictionary','{}'));
  end loop;
